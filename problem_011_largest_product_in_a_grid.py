...
def largest_grid_product(grid, adj_num):
    max_product = 0
    num_rows = len(grid)
    len_row = len(grid[0])
    last_row = num_rows - adj_num
    last_col = len_row - adj_num
    ...
    for row in range(num_rows):
        for col in range(len_row):
            ...
            if col <= last_col:
                ...
                if product > max_product: max_product = product 
            # next do diagonal downward
            if row <= last_row and col <= last_col:
                product = 1
                for i in range(adj_num):
                    product *= grid[row+i][col+i]
                if product > max_product: max_product = product
            # last do diagonal upward
            if row >= adj_num-1 and col <= last_col:
                product = 1
                for i in range(adj_num):
                    product *= grid[row-i][col+i]
                if product > max_product: max_product = product
...
# You are the 233482nd person to have solved this problem.
